...
const Task = () => {
    ...
    return (
        <div className={classes.wrapper}>
            <Card>
                <Typography className={classes.header} variant="h6">Выбор устройства</Typography>
                <Select options={categoryOptions} value={category} onChange={onChangeCategory} />
            </Card>
            <Slide direction="down" in={isCategoryChosen} mountOnEnter unmountOnExit>
                <Card>
                    <button onClick={onClearCategory} className={cnb(classes.clearButton, classes.button)}>
                        <ArrowBack />
                    </button>
                    <List items={category?.devices} onChange={onChangeDetailedCategory} />
                </Card>
            </Slide>
            <Slide direction="down" in={isDetailedDeviceChosen} mountOnEnter unmountOnExit>
                <Card>
                    <Grid container direction="column" alignItems="center">
                        <Grid container item alignItems="flex-start">
                            ...
                            <button onClick={onClearDetailedDevice} className={cnb(classes.closeButton, classes.button)}>
                                <Close className={classes.closeButtonIcon} />
                            </button>
                        </Grid>
                        <Grid item>
                            <img src={detailedDevice?.img} alt={detailedDevice?.label} className={classes.deviceImage} />
                        </Grid>
                        <Grid item>
